...
- `gui/`
  ...
  - `mainframe.py`: Main UI, feed refresh orchestration, list rendering, notifications, and menu actions.
    ...
    - Global media shortcuts (handled in `MainFrame.on_char_hook` and `PlayerFrame.on_char_hook`, NOT `\t` menu accelerators, so they work in both windows without double-firing): `Ctrl+Space` play/pause, `Ctrl+Shift+Space` stop, `Ctrl+Arrow` seek/volume. All of these are suppressed while a text control is focused (`_is_text_input_focused`) so screen-reader word/paragraph navigation in the article `content_ctrl` keeps working during playback — do not remove that guard.
