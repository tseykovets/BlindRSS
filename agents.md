...
- `core/`
  ...
  - `article_extractor.py`: Full-text extraction (trafilatura primary, BeautifulSoup fallback), pagination merge, boilerplate cleanup.
    - Current publisher rules: Sky News' bare Akamai "Powered and protected by" response is a block page (never article text); Fraser Valley Today and Fraser Valley Current/Beehiiv carry fuller bodies in RSS than on their public pages, so prefer their RSS body; strip trailing membership/promotional sections from 9to5Mac/9to5Google/9to5Toys and The Post Millennial; remove Rebel News `posts-profile` author biography sections before extraction.
