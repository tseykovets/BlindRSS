...
class AccessibleBrowserFrame(wx.Frame):
    ...
    def _finish_load_articles_page(self, view_id, offset, page, total):
        ...
        if total is None:
            self.status_lbl.SetLabel(_("Loaded {count} article(s).").format(count=loaded))
        else:
            self.status_lbl.SetLabel(_("Loaded {count} of {total} article(s).").format(count=loaded, total=int(total)))
